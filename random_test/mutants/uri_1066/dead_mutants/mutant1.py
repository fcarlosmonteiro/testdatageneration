...
def func(item1,item2,item3,item4,item5):
    ...
    negative=0
    for i in range(5):
        if not (list[i] % 2 == 0): #PM | type_kill=weakly args=[1022, 3988.3, -335.95, -6417.09, -8852.94] | type_kill=strongly args=[1022, 3988.3, -335.95, -6417.09, -8852.94] | type_kill=random args=[-490.0881034992599, -9342.623340626606, 8182.2578698217985, 7701.023038137384, -1303.2977937129217]
            even = even+1
        else:
            odd =odd+1
        if (list[i] > 0):
