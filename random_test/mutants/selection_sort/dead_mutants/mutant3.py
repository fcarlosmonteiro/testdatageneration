...
def selection_sort(item1,item2,item3,item4,item5):
    ...
    while i>=0 and i<=(len(list)-1):
        while j>=i and j<=(len(list)-1):          
            if not (list[i] > list[j]): #PM | type_kill=weakly args=[-8065.69, -5176.6, 2542.86, -8596.88, -4756.22] | type_kill=strongly args=[-8065.69, -5176.6, 2542.86, -8596.88, -4756.22] | type_kill=random args=[3120.912559955186, -4881.723783089147, 6778.865895311537, 1930.774145906218, 5929.16623460013]
                t=list[i]
                list[i]=list[j]
                list[j]=t
            j+=1
