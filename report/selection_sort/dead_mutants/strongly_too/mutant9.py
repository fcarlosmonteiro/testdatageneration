...
def selection_sort(item1,item2,item3,item4,item5):
    ...
    while i>=0 and i<=(len(list)-1):
        while (j <= i and j <= len(list) - 1): #PM | type_kill=weakly args=[-8065.69, -5176.6, 2542.86, -8596.88, -4756.22] | type_kill=strongly args=[-8065.69, -5176.6, 2542.86, -8596.88, -4756.22]
            if list[i]>list[j]:
                t=list[i]
                list[i]=list[j]
                list[j]=t
